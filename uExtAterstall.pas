...
type
  TfExtAterstall = class(TForm)
    ...
    siLangLinked_fExtAterstall: TsiLangLinked;
...
procedure TfExtAterstall.CreateCo ;
var
  Save_Cursor:TCursor;
begin
...
procedure TfExtAterstall.CloseDataSets;
begin
   With dmPkgs do
   Begin
...
procedure TfExtAterstall.lbCloseClick(Sender: TObject);
begin
 Close ;
end;

procedure TfExtAterstall.AddPkgToGrid(Sender: TObject;PkgNo : Integer;PkgSupplierCode : String3;ProductNo : Integer)  ;
var
 x : Integer ;
 Res_UserName : String ;
Begin
...
//     sq_OneUniquePkgExt.ParamByName('UserCompanyLoggedIn').AsInteger := ThisUser.CompanyNo ;
     ...
     sq_OneUniquePkgExt.Open ;
     if not sq_OneUniquePkgExt.Eof then
     Begin
     if dmsSystem.Pkg_Reserved(
          sq_OneUniquePkgExtPACKAGENO.AsInteger,
          sq_OneUniquePkgExtSUPP_CODE.AsString, Self.Name, Res_UserName
          ) = ThisUser.UserName+'/'+Self.Name { NO_USER_HAS_THIS_PACKAGE_RESERVED }then
     begin

      Try
      For x := 0 to 21 do
       mtLoadPackages.Fields.Fields[x].AsVariant:= sq_OneUniquePkgExt.Fields.Fields[x].AsVariant ;
...
//      mtLoadPackages.Post ;
      ...
      Begin
       ShowMessage(siLangLinked_fExtAterstall.GetTextOrDefault('IDS_0' (* 'Paketnr ' *) )+sq_OnePkgDetailDataPACKAGENO.AsString+'/'+sq_OnePkgDetailDataSUPP_CODE.AsString+siLangLinked_fExtAterstall.GetTextOrDefault('IDS_1' (* ' är reserverat av användare ' *) )+Res_UserName) ;
      End ;
     End  //if not...
     else
      Begin
       mtLoadPackages.Cancel ;
       ShowMessage(siLangLinked_fExtAterstall.GetTextOrDefault('IDS_0' (* 'Paketnr ' *) )+IntToStr(PkgNo)+'/'+PkgSupplierCode+siLangLinked_fExtAterstall.GetTextOrDefault('IDS_3' (* ' finns inte' *) )) ;
      End ;
...
Begin
 ...
 Begin
  Try
     ...
     sq_OneUniquePkgExt.ParamByName('PackageNo').AsInteger:= PkgNo ;
     sq_OneUniquePkgExt.ParamByName('SupplierCode').AsString:= PkgSupplierCode ;
     sq_OneUniquePkgExt.ParamByName('OwnerNo').AsInteger:= SupplierNo ;
     sq_OneUniquePkgExt.ParamByName('UserCompanyLoggedIn').AsInteger:= ThisUser.CompanyNo ;
     sq_OneUniquePkgExt.Open ;
     if not sq_OneUniquePkgExt.Eof then
     Begin
...
function TfExtAterstall.IdentifyPackageSupplier(
  const PkgNo : Integer;
  var PkgSupplierCode: String3;
  Var ProductNo : Integer;
  Var Res_UserName : String) : TEditAction;
...
//    PkgSupplierCode := dmsSystem.NonActivePkgNoToSuppCode(PkgNo, mtUserPropPIPNo.AsInteger, mtUserPropOwnerNo.AsInteger, SupplierNo, ProductNo);
    PkgSupplierCode := dmsSystem.GetNonActivePkgNoExt(PkgNo, mtUserPropPIPNo.AsInteger, mtUserPropOwnerNo.AsInteger, SupplierNo, ProductNo, mtUserPropSupplierCode.AsString);

    if ((PkgSupplierCode = '___') or (PkgSupplierCode = '')) then
    Begin
      Result := eaREJECT;
    End
    else
...
procedure TfExtAterstall.FormCreate(Sender: TObject);
begin
...
procedure TfExtAterstall.FormCloseQuery(Sender: TObject;var CanClose: Boolean);
begin
 CanClose:= True ;
...
procedure TfExtAterstall.lbPkgInfoClick(Sender: TObject);
...
procedure TfExtAterstall.InsertPkgSerie(Sender: TObject) ;
 ...
 Begin
  ...
  Try
   ...
   Begin
    ...
    ResultButton:= MessageDlg(siLangLinked_fExtAterstall.GetTextOrDefault('IDS_4' (* 'Upp till ' *) )+IntToStr(NoOfPkgsInSerie)+siLangLinked_fExtAterstall.GetTextOrDefault('IDS_5' (* ' paket kanske hämtas, är det korrekt?' *) ),
...
procedure TfExtAterstall.FormDestroy(Sender: TObject);
begin
 FreeAndNil(dmPkgs) ; //Free ;
end;

procedure TfExtAterstall.bbRemoveAllRowsClick(Sender: TObject);
begin
 //Remove all rows
 With dmPkgs do
 Begin
...
procedure TfExtAterstall.bbClearPkgGridClick(Sender: TObject);
begin
 bbRemoveAllRowsClick(Sender) ;
end;

procedure TfExtAterstall.RemoveAllSuccespkgs(Sender: TObject);
begin
 //Remove all rows
 With dmPkgs do
 Begin
...
procedure TfExtAterstall.ePktNrKeyPress(Sender: TObject; var Key: Char);
begin
 With dmPkgs do
 Begin
  if Key = #13 then
...
procedure TfExtAterstall.ScanningPkgNo(Sender: TObject; PkgNo : String) ;
const
  LF = #10;
var
  NewPkgNo        : Integer ;
...
//Notera att i den långa koden skall supplier koden finnas!
  ...
  Begin
   ...
   if NewPkgNo = 0 then
   Begin
    ShowMessage(siLangLinked_fExtAterstall.GetTextOrDefault('IDS_9' (* 'Streckkoden kunde inte översättas till ett Paketnr' *) )) ;
    Exit ;
   End ;
   ...
   Begin
    ShowMessage(siLangLinked_fExtAterstall.GetTextOrDefault('IDS_10' (* 'Inget paket kunde identifieras' *) )) ;
    Exit ;
   End ;
...
//   else
  ...
  End
   ...
   if Action = eaREJECT then
    Begin
     ShowMessage(siLangLinked_fExtAterstall.GetTextOrDefault('IDS_0' (* 'Paketnr ' *) )+IntToStr(NewPkgNo)+'/'+PkgSupplierCode+siLangLinked_fExtAterstall.GetTextOrDefault('IDS_3' (* ' finns inte' *) )) ;
    End
    else
     if Action = eaReserved then
      Begin
       ShowMessage(siLangLinked_fExtAterstall.GetTextOrDefault('IDS_0' (* 'Paketnr ' *) )+IntToStr(NewPkgNo)+'/'+PkgSupplierCode+siLangLinked_fExtAterstall.GetTextOrDefault('IDS_1' (* ' är reserverat av användare ' *) )+Res_UserName) ;
      End
      else
       if Action = eaAlreadyAvReg then
       Begin
        ShowMessage(siLangLinked_fExtAterstall.GetTextOrDefault('IDS_0' (* 'Paketnr ' *) )+IntToStr(NewPkgNo)+'/'+PkgSupplierCode+siLangLinked_fExtAterstall.GetTextOrDefault('IDS_16' (* ' är redan avregistrerat mot mätpunkt ' *) )+RegPointName) ;
       End ;
...
function TfExtAterstall.AddPkgTo_mtPackage(Sender: TObject;const LIP, PkgNo : Integer;PkgSupplierCode : String3;ProductNo : Integer) : TEditAction ;
var
 x : Integer ;
// Res_UserName : String ;
Begin
...
procedure TfExtAterstall.ScanningEgnaPkgNo(Sender: TObject; PkgNo : String) ;
const
  LF = #10;
var
  NewPkgNo          : Integer ;
...
//   dmPkgs.mtLoadPackages.IndexName:= 'mtLoadPackagesIndex5' ;
  ...
  Begin
   ...
   if NewPkgNo = 0 then
   Begin
    ShowMessage(siLangLinked_fExtAterstall.GetTextOrDefault('IDS_19' (* 'Koden kunde inte översättas till ett Paketnr' *) )) ;
    Exit ;
   End ;
  End ;
...
procedure TfExtAterstall.mtUserPropOwnerNoChange(Sender: TField);
begin
...
procedure TfExtAterstall.mtUserPropPIPNoChange(Sender: TField);
begin
 With dm_UserProps do
 Begin
...
procedure TfExtAterstall.mtUserPropProducerNoChange(Sender: TField);
begin
 With dm_UserProps do
 Begin
...
procedure TfExtAterstall.mtUserPropAfterInsert(DataSet: TDataSet);
begin
 mtUserPropRegDate.AsDateTime           := Now ;
 mtUserPropProductGroupNo.AsInteger     := -1 ;
 mtUserPropProductNo.AsInteger          := -1 ;
 mtUserPropProductDescription.AsString  := siLangLinked_fExtAterstall.GetTextOrDefault('IDS_28' (* 'Ingen ändring' *) ) ;
end;

procedure TfExtAterstall.grdPkgsDBBandedTableView1PACKAGENOPropertiesValidate(
  Sender: TObject; var DisplayValue: Variant; var ErrorText: TCaption;
  var Error: Boolean);
var
  NewValue          : String;
...
{  if Action = eaACCEPT then
  ...
  End
   ...
   if ((Action = eaREJECT) and (Length(Trim(PkgSupplierCode)) = 0)) then
    Begin
     ErrorText:= siLangLinked_fExtAterstall.GetTextOrDefault('IDS_0' (* 'Paketnr ' *) ) + NewValue + '/' + mtUserPropSupplierCode.AsString + siLangLinked_fExtAterstall.GetTextOrDefault('IDS_30' (* ' är inte tillgängligt' *) ) ;
     Error:= True ;
    End
    else
     if Action = eaReserved then
     Begin
      ErrorText:= siLangLinked_fExtAterstall.GetTextOrDefault('IDS_0' (* 'Paketnr ' *) ) + NewValue + siLangLinked_fExtAterstall.GetTextOrDefault('IDS_1' (* ' är reserverat av användare ' *) ) + Res_UserName ;
      Error:= True ;
     End
      else
       if Action = eaAlreadyAvReg then
       Begin
        ShowMessage(siLangLinked_fExtAterstall.GetTextOrDefault('IDS_0' (* 'Paketnr ' *) )+NewValue+'/'+PkgSupplierCode+siLangLinked_fExtAterstall.GetTextOrDefault('IDS_16' (* ' är redan avregistrerat mot mätpunkt ' *) )+RegPointName) ;
       End ;
  finally
    Screen.Cursor := Save_Cursor;  { Always restore to normal }
  end;
...
procedure TfExtAterstall.acAvregistreraPaketExecute(Sender: TObject);
const
  LF = #10;
Begin
 if mtUserPropRegDate.AsDateTime > Now then
 Begin
  ShowMessage(siLangLinked_fExtAterstall.GetTextOrDefault('IDS_35' (* 'Registreringsdatum får inte vara större än aktuellt datum' *) ));
  Exit ;
 End ;

 if (mtUserPropLIPNo.AsInteger < 1) or (mtUserPropLIPNo.IsNull) then
 Begin
  ShowMessage(siLangLinked_fExtAterstall.GetTextOrDefault('IDS_36' (* 'Lagergrupp saknas att aktivera paket till, var vänlig välj en lagergrupp.' *) ));
  Exit ;
 End ;
 ...
 Begin
  if mtUserProp.State in [dsEdit, dsInsert] then
   mtUserProp.Post ;
  if MessageDlg(siLangLinked_fExtAterstall.GetTextOrDefault('IDS_37' (* 'Vill du aktivera valda paket? ' *) )
  ,    mtConfirmation, [mbYes, mbNo], 0) = mrYes then
  Begin
   if dmPkgs.AterStallPktTillInventoryEXT(mtUserProp, mtUserPropLIPNo.AsInteger, mtUserPropOwnerNo.AsInteger) then
    RemoveAllSuccespkgs(Sender) ;
  End ; //if
 End
 else
 ShowMessage(siLangLinked_fExtAterstall.GetTextOrDefault('IDS_38' (* 'Paket, markerade med röd färg, kan inte aktiveras pga att registreringsdatum är före maxdatum i en inventering där lagergruppen ingår.' *) )) ;
end;

procedure TfExtAterstall.acPkgNoSerieExecute(Sender: TObject);
begin
 With dmPkgs do
 Begin
...
procedure TfExtAterstall.acStartBarCodeScanningExecute(Sender: TObject);
begin
//Proc GetPkgPos läser in paketnr posistioner och längder
 ...
 Try
  fScanPkgNo.cbEgenLevKod.Caption:= siLangLinked_fExtAterstall.GetTextOrDefault('IDS_39' (* 'Scanna endast in paket med leverantörskod ' *) )+EgenPkgSupplierCode ;
...
procedure TfExtAterstall.acCloseExecute(Sender: TObject);
begin
 Close ;
end;

procedure TfExtAterstall.acAddRowExecute(Sender: TObject);
begin
 dmPkgs.mtLoadPackages.Append ;
 grdPkgs.SetFocus ;
end;

procedure TfExtAterstall.acRemoveRowExecute(Sender: TObject);
begin
 // remove one row
 With dmPkgs do
 if ((mtLoadPackages.Active) AND (mtLoadPackages.RecordCount > 0))
...
procedure TfExtAterstall.acRemoveRowUpdate(Sender: TObject);
begin
 // remove one row
 With dmPkgs do
...
procedure TfExtAterstall.acAddRowUpdate(Sender: TObject);
begin
...
procedure TfExtAterstall.acAvregistreraPaketUpdate(Sender: TObject);
begin
 With dmPkgs do
 Begin
...
procedure TfExtAterstall.RemotePkgEntry (PkgNos : TkbmMemTable) ;
Var     //NoOfPkgsInSerie,
        x                   : Integer ;
        ResultButton        : word ;
        Save_Cursor         : TCursor;
   ...
   Begin
    ...
    Begin
     ...
     Begin
      ...
      sq_OneUniquePkgExt.Close ;
      sq_OneUniquePkgExt.ParamByName('PackageNo').AsInteger           := PkgNos.FieldByName('PACKAGENO').AsInteger ;
      sq_OneUniquePkgExt.ParamByName('SupplierCode').AsString         := PkgNos.FieldByName('SUPP_CODE').AsString ;
      sq_OneUniquePkgExt.ParamByName('OwnerNo').AsInteger             := PkgNos.FieldByName('OwnerNo').AsInteger ;
      sq_OneUniquePkgExt.ParamByName('UserCompanyLoggedIn').AsInteger := ThisUser.CompanyNo ;
      sq_OneUniquePkgExt.ParamByName('Status').AsInteger              := 1 ;
      sq_OneUniquePkgExt.Open ;
      mtLoadPackages.Insert ;
      For x := 0 to 21 do
       mtLoadPackages.Fields.Fields[x].AsVariant:= sq_OneUniquePkgExt.Fields.Fields[x].AsVariant ;
    ...
    End  //if dmsSystem.Pkg_Reserved(
     ...
     Begin
      ShowMessage(siLangLinked_fExtAterstall.GetTextOrDefault('IDS_0' (* 'Paketnr ' *) )+PkgNos.FieldByName('PACKAGENO').AsString+'/'+PkgNos.FieldByName('SUPP_CODE').AsString+siLangLinked_fExtAterstall.GetTextOrDefault('IDS_41' (* ' är reserverad av ' *) )+Res_UserName) ;
     End ;
...
procedure TfExtAterstall.acRemoveAllRowsExecute(Sender: TObject);
begin
 // remove one row
 With dmPkgs do
 if ((mtLoadPackages.Active) AND (mtLoadPackages.RecordCount > 0))
...
procedure TfExtAterstall.acRemoveAllRowsUpdate(Sender: TObject);
begin
 With dmPkgs do
...
function TfExtAterstall.ControlInvDate(Sender: TObject) : Boolean ;
const
  LF = #10;
Var
    LastInvNr   : Integer ;
...
procedure TfExtAterstall.grdPkgsDBBandedTableView1StylesGetContentStyle(
  Sender: TcxCustomGridTableView; ARecord: TcxCustomGridRecord;
  AItem: TcxCustomGridTableItem; var AStyle: TcxStyle);
var
