...
type
  TfrmSelectLO_NumberSSP = class(TForm)
    ...
    procedure bbCustomizeLayoytClick(Sender: TObject);
...
{  if Key <> VK_RETURN then Exit;
 ...
 Begin
   ...
   Begin
    ...
    grdLO.SetFocus ;
   End ;
//  cdsAvrop.FindKey .Locate('LO',nfSearchLO.AsInteger, []) ;
 End ; //with }
 ...
 Begin
  ...
  Begin
   ...
   if cds_LO_LookUp.FindKey([StrToInt(Trim(meLO_No.Text))]) then
    bbOK.SetFocus ;
  End ;
//   .Locate('LONumber',StrToInt(Trim(beSearchLO.Text)), []) ;
  meLO_No.Text:= '' ;
//  meLO_No.SetFocus ;
//  bbOK.SetFocus ;
...
begin
// if dmLoadEntrySSP.cds_LO_LookUp.Filter:= 'ORDERTYPE = ''SALES''' ;
...
//        grdLOCUSTOMER.Caption:= 'EXT_CUSTOMER' ;
//        grdLOSUPPLIER.Caption:= 'LOCAL_SUPPLIER' ;
//        grdLOLOCAL_CUST.Caption:= 'LOCAL_CUSTOMER' ;
      End ;
  1 : Begin
        dmLoadEntrySSP.cds_LO_LookUp.Filter:= 'SHIPPING = 1' ;
        dmLoadEntrySSP.cds_LO_LookUp.Filtered:= True ;
        grdLookupDBTableView1CUSTOMER.Caption:= 'Extern leverantör' ;
//        grdLOCUSTOMER.Caption:= 'EXT_SUPPLIER' ;
//        grdLOSUPPLIER.Caption:= 'LOCAL_SUPPLIER' ;
//        grdLOLOCAL_CUST.Caption:= 'LOCAL_CUSTOMER' ;
      End ;
  2 : Begin
...
procedure TfrmSelectLO_NumberSSP.bbCustomizeLayoytClick(Sender: TObject);
begin
// grdLO.ColumnsCustomizing ;
end;

procedure TfrmSelectLO_NumberSSP.FormCreate(Sender: TObject);
begin
// grdLO.LoadFromIniFile(dmsConnector.InifilesMap+'VisLONOSSP'+'.'+ThisUser.UserName);
end;

procedure TfrmSelectLO_NumberSSP.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
// grdLO.SaveToIniFile(dmsConnector.InifilesMap+'VisLONOSSP'+'.'+ThisUser.UserName);
 CanClose:= True ;
end;
