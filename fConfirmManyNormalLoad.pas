...
Begin
 ...
 Begin
  ...
  Except
   On E: Exception do
   Begin
   ShowMessage(E.Message+' :sq_ErrorPkgs.ExecSQL') ;
   Raise ;
   End ;
  End ;
 End ;
...
//Ändra lager ställe på AR paket
   ...
   if Sales_LONo > 0 then
    NewLoadNo:= dmsSystem.CopyPOLoadToSalesLoadAndSetPackagesAsNotAvailable(mtLoadShippingPlanNewLoadNo.AsInteger, Sales_LONo, ConfirmedOrigLoadNo, 0)
    else
     NewLoadNo:= 0 ;
   if NewLoadNo > 0 then
...
procedure TformConfirmanyNormalLoad.FormCloseQuery(Sender: TObject;
...
begin
 ...
 dmArrivingLoads.sq_PkgNumber.Close ;
