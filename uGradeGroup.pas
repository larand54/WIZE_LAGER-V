...
Class procedure TfGradeGroup.Execute (const LIPNo, ClientNo : Integer) ;
Begin
...
begin
 ...
 Begin
  ...
  cds_GradeGroupHdrLIPNo.AsInteger:= FLIPNo ;  
 End ;
end;
...
Begin
 ...
 Begin
  cds_Grade_SV.Active:= False ;
  sq_Grade_SV.ParamByName('ClientNo').AsInteger     := cds_GradeGroupHdrClientNo.AsInteger ; //FClientNo ;
  sq_Grade_SV.ParamByName('GradeGroupNo').AsInteger := cds_GradeGroupHdrGradeGroupNo.AsInteger ;
  cds_Grade_SV.Active:= True ;
 End ;//with 
End ;

procedure TfGradeGroup.acDeleteGroupUpdate(Sender: TObject);
begin
 With dmInvCtrl do
 Begin
  acDeleteGroup.Enabled:= (cds_GradeGroupHdr.Active) and (cds_GradeGroupHdr.RecordCount > 0) ;
 End ;
end;
