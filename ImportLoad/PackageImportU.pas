...
Function GetAlpha(Identifier : String) : String ;//By Lars
Var x : Integer ;
Begin
 Result:= '' ;
 For x := 1 to Length(Identifier) do
 Begin
  if not StrToIntDef(Copy(Identifier,x,1),-1) in [0..9] then
   Result := Result + Copy(Identifier,x,1) ;
 End ;
End ;
...
begin
  Result := Trim(mw.DeliveryMessageWoodHeader.DeliveryMessageNumber) ;
end;
...
//LARS added "filter" to only get identifier of type Primary!
          ...
          dsInfo.FieldByName('ProductIdentifier').AsString              := sProductIdentifier;
          dsInfo.FieldByName('ProductDescription').AsString             := sProductDescription ;
          dsInfo.FieldByName('Prefix').AsString                         := sPrefix ;
          dsInfo.Post;
        end;
