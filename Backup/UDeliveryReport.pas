...
uses
  ...
  dxPScxPivotGridLnk, dxPScxSSLnk, dxSkinsdxRibbonPainter ;
...
procedure TfDeliveryReport.acPkgInfoPkgNosTableExecute(Sender: TObject);
var frmPkgInfo: TfrmPkgInfo;
begin
 With dmInventory do
 Begin
...
procedure TfDeliveryReport.acRefreshProdSUMExecute(Sender: TObject);
begin
 With dmInventory do
 Begin
...
Begin
 ...
 Begin
  ...
  Begin
    ...
    SQL.Add('SUM(pp.[Värde]) AS NETTO, va.VarugruppNamn, pp.LO') ;
...
//    if deStartPeriod.EditValue > 0 then
    SQL.Add('WHERE pp.Utlastad >= ' + QuotedStr(DateTimeToStr(deDBStartPeriod.Date))) ;

    if deDBEndPeriod.EditValue > 0 then
    SQL.Add('AND pp.Utlastad <= ' + QuotedStr(DateTimeToStr(deDBEndPeriod.Date))) ;
...
procedure TfDeliveryReport.Build_ProductionByProduct ;
Begin
 With dmInventory do
 Begin
...
//    SQL.Add('Inner Join dbo.LengthSpec LS ON LS.LengthSpecNo = pt.LengthSpecNo') ;
    ...
    SQL.Add(GetSQLofComboFilter(1, 'pg.NominalThicknessMM', ccbNT)) ;
    SQL.Add(GetSQLofComboFilter(1, 'pg.NominaWidthMM', ccbNB)) ;
