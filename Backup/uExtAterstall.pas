...
begin
 ...
 Begin
  ...
  if ThisUser.CompanyNo = cVidaPackaging then
  Begin
   sq_PIP.ParamByName('UserID').AsInteger:= ThisUser.UserID ;
  End
  else
  Begin
   sq_PIP.ParamByName('UserID').AsInteger:= -1 ;
  End ;  
...
Begin
 ...
 Begin
  if mtUserProp.State in [dsEdit, dsInsert] then
   mtUserProp.Post ;
  if MessageDlg('Vill du återställa valda paket? '
