...
begin
  ...
  FShowEmptyText := True;
  FEmptySelectionText := cxGetResourceStringNet(cxSCheckComboBoxEmptySelectionText);
...
begin
  Result := FEmptySelectionText <>
     cxGetResourceStringNet(cxSCheckComboBoxEmptySelectionText);
end;
...
{------------------------------------------------------------------------------}
procedure TcxCustomCheckComboBox.KeyDown(var Key: Word;
  Shift: TShiftState);
begin
  if ( (Key = VK_RETURN) or (Key = VK_DELETE) ) and HasPopupWindow then
  begin
    (LookupData.ActiveControl as TcxCustomCheckComboBoxListBox).KeyDown(Key, Shift);
  end
  else
    inherited;
end;

procedure TcxCustomCheckComboBoxListBox.KeyDown(var Key: Word;
  Shift: TShiftState);
begin
  if (ItemIndex <> -1) and Assigned(Edit) then
  begin
    if (Key = VK_RETURN) or (Key = VK_DELETE) then
    begin
      if not (Edit.ActiveProperties.Items[ItemIndex].Enabled) then Exit;
      if ( Edit.FStates[ItemIndex] = cbsChecked ) and (Key = VK_RETURN) then Exit;
      if ( Edit.FStates[ItemIndex] = cbsUnChecked ) and (Key = VK_DELETE) then Exit;
