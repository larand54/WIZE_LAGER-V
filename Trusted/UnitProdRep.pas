...
//  cds_PropsLIPNo.AsInteger              := integer(peLogicalInventory.Items.Objects[peLogicalInventory.ItemIndex]) ;
  ...
  cds_PropsRegPointNo.AsInteger         := peSupplier.ItemIndex ;
...
begin
 ...
 Begin
  ...
  if thisuser.UserID = 8 then
    sq_Temp_Inventory.SQL.SaveToFile('prod_By_LengthFilter.txt');
  sq_Temp_Inventory.ExecSQL(True) ;
  sq_Temp_Inventory.SQL.Clear ;  
...
//  sq_Temp_Inventory.SQL.Add('AND pp.ProductionDate <= :EndDate') ;
  ...
  if thisuser.UserID = 8 then
  sq_Temp_Inventory.SQL.SaveToFile('sq_PROD_By_Length.txt') ;
...
//  sq_Temp_Inventory.SQL.Add('AND pp.ProductionDate <= :EndDate') ;
  ...
  if thisuser.UserID = 8 then
   sq_Temp_Inventory.SQL.SaveToFile('sq_prod_By_Pkgno.txt');
//  sq_Temp_Inventory.ParamByName('StartDate').AsSQLTimeStamp:= DateTimeToSQLTimeStamp(StartDate.Date) ;
//  sq_Temp_Inventory.ParamByName('EndDate').AsSQLTimeStamp:= DateTimeToSQLTimeStamp(EndDate.Date) ;
  sq_Temp_Inventory.ExecSQL(False) ;
 End ; //with
