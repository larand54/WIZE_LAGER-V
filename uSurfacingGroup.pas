...
uses
  ...
  cxLabel, cxLookAndFeels, cxLookAndFeelPainters, FMTBcd, DBClient, Provider,
  SqlExpr;
...
type
  TfSurfacingGroup = class(TForm)
    ...
    procedure acDeleteGroupUpdate(Sender: TObject);
  private
  ...
  public
    ...
    FLIPNo,
    FClientNo : Integer ;
    Class procedure Execute (const LIPNo, ClientNo : Integer) ;
  end;
...
Class procedure TfSurfacingGroup.Execute (const LIPNo, ClientNo : Integer) ;
Begin
...
begin
 ...
 Begin
  ...
  cds_SurfGrpHdrLIPNo.AsInteger     := FLIPNo ;
 End ;
end;
...
Begin
 ...
 Begin
  cds_Surfacing_SV.Active:= False ;
  sq_Surfacing_SV.ParamByName('ClientNo').AsInteger         := cds_SurfGrpHdrClientNo.AsInteger ;// FClientNo ;
  sq_Surfacing_SV.ParamByName('SurfacingGroupNo').AsInteger := cds_SurfGrpHdrSurfacingGroupNo.AsInteger ;
...
procedure TfSurfacingGroup.acDeleteGroupUpdate(Sender: TObject);
begin
 With dmInvCtrl do
 Begin
  acDeleteGroup.Enabled:= (cds_SurfGrpHdr.Active) and (cds_SurfGrpHdr.RecordCount > 0) ;
 End ;
end;
